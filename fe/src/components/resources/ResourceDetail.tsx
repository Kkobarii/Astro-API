...
export function loader({ params }: any) {
  const resourceUrl = Url + "resources/" + params.resourceId + "?include=planets";

  // Fetch resource data
  return fetch(resourceUrl)
    .then((response) => response.json())
    .then((resourceData) => {
      const planetIds = resourceData.planets.map((resource: any) => resource.planetId);
      ...
      return Promise.all(planetPromises).then((planets) => {
        ...
        planets.forEach((planet: any) => {
          const location = resourceData.planets.find((p: any) => p.planetId === planet.id);
...
export function ResourceDetail () {
  ...
  return (
    <div className="detail">
      <Stack direction="row">
        <Container>
          <Typography variant="h2" gutterBottom className="name">{resource.name}</Typography>
          <Typography variant="body1" className="description">{resource.description}</Typography>
        </Container>
        <div>
      ...
      <Stack textAlign="left">
        {planets && planets.length > 0 && (
            <Container>
            <Typography variant="h4" gutterBottom style={{marginTop: "1em"}}>Planets:</Typography>
                  ...
                  <TableBody>
                    {planets.map((planet: any) => (
                      <StyledTableRow key={planet.id}>
                        <StyledTableCell>
                          <Button component={NavLink} to={`/planets/${planet.id}`}>
