...
export function loader({ params }: any) {
  ...
  return fetch(planetUrl)
    ...
    .then((planetData) => {
      const resourceIds = planetData.resources.map(
      ...
      const resourcePromises = resourceIds.map((resourceId: any) =>
        fetch(Url + "/resources/" + resourceId).then((response) =>
          response.json()
        )
      );
      ...
      return Promise.all([
        ...
        resources.forEach((resource: any) => {
          const location = planetData.resources.find(
            (r: any) => r.resourceId === resource.id
          );
        ...
        gases.forEach((gas: any) => {
          const partsPerUnit = planetData.gases.find(
            (g: any) => g.gasId === gas.id
          );
